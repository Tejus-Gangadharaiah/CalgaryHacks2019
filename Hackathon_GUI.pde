...
void draw() {
  ...
  if (addnode == 1 && mouseY > 1.2*height/8){
    fill(255);
...
void new_road(){
  ...
  int s = -1, d = -1, t = -1, p = -1;
  if (roadd == 0){
      final String s1 = showInputDialog(null, "Please enter the source node");
      s = int(s1);              // the idea is if a user inputs "-1" they can choose the nodes 
...
void edit_road(){
  if (editroad == 0){
    showMessageDialog(null, "Select the road to edit");
  }
  if (editroad == 1){
    addroad = 0;
    addnode = 0;
